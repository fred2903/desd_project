...
% test
The implemented design demonstrates the timing analysis yields compliant results for both setup and hold constraints at the target operating frequency of 150~MHz.

Regarding the setup time, the most critical path is located within the \texttt{moving\_average\_0} block (Path 21). While the design requirement demands a 150~MHz clock, this implementation achieves a positive slack of 0.190~ns. This safety margin allows the circuit to potentially operate at a maximum frequency of approximately 154~MHz.

Concerning the hold time, the critical path is identified along the data route from the \texttt{diligent\_jstk2\_0} module to the \texttt{effect\_selector\_0} unit (Path 31). The required hold time constraint is 0~ns, and the implementation guarantees a safe positive hold slack of 0.026~ns, ensuring that no race conditions occur.
...
At the operating frequency of 150 MHz, the following implementation's power consumption is as shown in the following tables.
